...
`shell.ts`'s mount block runs **after** cable-hole drilling: tabs are unioned in via `Manifold.cube` first, then for each slot a **through-hole + back pocket** pair is subtracted. The through-hole spans Z ∈ `[0, backThickness]`; the pocket is a wider version (XY margin = `mountShankDiameter` on each side) recessed only half the panel depth (Z ∈ `[0, backThickness × 0.5]`) on the back face. The pocket gives the screw head clearance and a chamfered look. For flat-back (`backCavityDepth = 0`) both shapes go through the back panel directly. For open-back the tab fills the gap at the open rear so the through-hole has material to cut through; the partition further forward stays solid.
