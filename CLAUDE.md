...
`shell.ts`'s mount block runs **after** cable-hole drilling: tabs are unioned in via `Manifold.cube` first, then for each slot a **through-hole + interior pocket** pair is subtracted. The through-hole spans Z ∈ `[0, backThickness]`; the pocket is a wider version (XY margin = `mountShankDiameter` on each side) recessed half the panel depth on the **letter-interior face** (Z ∈ `[backThickness × 0.5, backThickness]`) — i.e., the side where the screw head ends up after passing all the way through the through-hole. For open-back, the head sits in the rear cavity behind the tab; for flat-back, it sits in the front cavity above the back panel. The pocket gives the head clearance and a clean visual finish. For flat-back (`backCavityDepth = 0`) both shapes go through the back panel directly. For open-back the tab fills the gap at the open rear so the through-hole has material to cut through; the partition further forward stays solid.
